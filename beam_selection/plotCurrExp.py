...
curve=np.load('KDExp/CurvesMIXTURE_BETA_8_ANTI010.npy')
...
curve1=np.load('KDExp/CurvesMIXTURE_BETA_8_CURR010.npy')
curve1=curve1/ curve1[0,curve.shape[1] - 1]
mean=np.mean(curve,axis=0)
std=np.sqrt(np.sum((curve-mean)**2,axis=0)/(curve.shape[0]-1))
plt.plot(np.arange(1,257), mean, label=r'Curriculum', color="yellowgreen")

curve2=np.load('KDExp/CurvesMIXTURE_BETA_8_VANILLA010.npy')
curve2=curve2/ curve2[0,curve.shape[1] - 1]
mean=np.mean(curve,axis=0)
std=np.sqrt(np.sum((curve-mean)**2,axis=0)/(curve.shape[0]-1))
