...
def NON_LOCAL_MIXTURE(FLATTENED, LIDAR_TYPE):
    if(LIDAR_TYPE=='CENTERED'):
        ...
        if(FLATTENED):
            ...
            input_lid = Input(shape=(60, 330, 10))
    noisy_input_lid=GaussianNoise(0.01)(input_lid)
    layer = Conv2D(8, kernel_size=(5, 5), activation='relu', padding="SAME", kernel_initializer=initializers.HeUniform)(noisy_input_lid)
    layer= Conv2D(8, kernel_size=(5, 5), strides=2, activation='relu', padding="SAME", kernel_initializer=initializers.HeUniform)(layer)
    ...
    layer = Conv2D(1, kernel_size=(3, 3), strides=(1, 1), activation='relu', padding="SAME", kernel_initializer=initializers.HeUniform)(non_local_layer)
    layer = Flatten()(layer)
    out_lid = Dense(16, activation='relu')(layer)
    '''GPS branch'''
    input_coord = Input(shape=(3))
...
def NON_LOCAL_MIXTURE_OLD(FLATTENED, LIDAR_TYPE):
    ...
    layer = Conv2D(1, kernel_size=(3, 3), strides=(1, 2), activation='relu', padding="SAME", kernel_initializer=initializers.HeUniform)(non_local_layer)
    layer = Flatten()(layer)
    out_lid = Dense(16, activation='relu')(layer)
    '''GPS branch'''
    input_coord = Input(shape=(3))
