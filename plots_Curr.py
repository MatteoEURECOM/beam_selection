...
curve=np.load('CurrExp/CurvesMIXTURE_BETA_8_ANTI.npy')
curve=curve/ curve[0,curve.shape[1] - 1]
...
plt.grid()
plt.legend(['Standard', 'Curriculum', 'Anticurriculum'], fontsize=12, loc='lower right')
...
curve=np.load('KDExp/CurvesMIXTURE_BETA_8_CURR010.npy')
curve=curve/ curve[0,curve.shape[1] - 1]
data_y_2=np.mean(curve,axis=0)

curve=np.load('CurrExp/CurvesMIXTURE_BETA_8_ANTI010.npy')
curve=curve/ curve[0,curve.shape[1] - 1]
data_y_3=np.mean(curve,axis=0)
...
plt.plot(np.arange(1, 31), data_y_1[0:30], marker='o', linewidth=1, markevery=2,markersize=4)
plt.plot(np.arange(1, 31), data_y_2[0:30], marker='^', linewidth=1, markevery=2,markersize=4)
plt.plot(np.arange(1, 31), data_y_3[0:30], marker='v', linewidth=1, markevery=2,markersize=4)# Grid, legend, axis labels. Use LaTeX math font if necessary.
plt.grid()
plt.legend(['Standard', 'Curriculum', 'Anticurriculum'], fontsize=12, loc='lower right')
plt.xlabel("$k$", fontsize=12)
plt.ylabel("Top-$k$ accuracy", fontsize=12)
